...
{   int nb = len >> 4;
    ...
    {   uint_8t *ksp = (uint_8t*)(ctx->ks);
        ...
        {   aligned_auto(uint_8t, buf, BFR_BLOCKS * _AES_BLOCK_SIZE, 16);
            uint_8t *ip, *op;
            ...
            while(nb)
            {
                ...
                if(ip != ibuf)
                    memcpy(buf, ibuf, m * _AES_BLOCK_SIZE);

                via_ecb_op5(ksp, cwd, ip, op, m);

                if(op != obuf)
                    memcpy(obuf, buf, m * _AES_BLOCK_SIZE);

                ibuf += m * _AES_BLOCK_SIZE;
                obuf += m * _AES_BLOCK_SIZE;
                nb -= m;
            }
        }
    ...
    while(nb--)
    {
        if(aes_encrypt(ibuf, obuf, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;
        ibuf += _AES_BLOCK_SIZE;
        obuf += _AES_BLOCK_SIZE;
    }
#endif
    return EXIT_SUCCESS;
}
...
{   int nb = len >> 4;
    ...
    {   uint_8t *ksp = (uint_8t*)(ctx->ks), *ivp = iv;
        aligned_auto(uint_8t, liv, _AES_BLOCK_SIZE, 16);
        via_cwd(cwd, hybrid, enc, 2 * ctx->inf.b[0] - 192);
    ...
    if(!ALIGN_OFFSET( ibuf, 4 ) && !ALIGN_OFFSET( iv, 4 ))
        while(nb--)
        {
            ...
            if(aes_encrypt(iv, iv, ctx) != EXIT_SUCCESS)
                return EXIT_FAILURE;
            memcpy(obuf, iv, _AES_BLOCK_SIZE);
            ibuf += _AES_BLOCK_SIZE;
            obuf += _AES_BLOCK_SIZE;
        }
    else
...
{   unsigned char tmp[_AES_BLOCK_SIZE];
    int nb = len >> 4;
...
# ifdef FAST_BUFFER_OPERATIONS
    if(!ALIGN_OFFSET( obuf, 4 ) && !ALIGN_OFFSET( iv, 4 ))
        while(nb--)
        {
            memcpy(tmp, ibuf, _AES_BLOCK_SIZE);
            if(aes_decrypt(ibuf, obuf, ctx) != EXIT_SUCCESS)
                return EXIT_FAILURE;
...
{   int cnt = 0, b_pos = (int)ctx->inf.b[2], nb;
    ...
    {
        while(b_pos < _AES_BLOCK_SIZE && cnt < len)
        {
            *obuf++ = (iv[b_pos++] ^= *ibuf++);
            cnt++;
        }

        b_pos = (b_pos == _AES_BLOCK_SIZE ? 0 : b_pos);
    }
    ...
    if((nb = (len - cnt) >> 4) != 0)    /* process whole blocks */
    {
        ...
        {   int m;
            ...
            if(!ALIGN_OFFSET( ibuf, 16 ) && !ALIGN_OFFSET( obuf, 16 ))
            {
                ...
                ibuf += nb * _AES_BLOCK_SIZE;
                obuf += nb * _AES_BLOCK_SIZE;
                cnt  += nb * _AES_BLOCK_SIZE;
            }
...
# endif
    ...
    while(cnt < len)
    {
        ...
        while(cnt < len && b_pos < _AES_BLOCK_SIZE)
        {
            *obuf++ = (iv[b_pos++] ^= *ibuf++);
            cnt++;
        }
...
#define BFR_LENGTH  (BFR_BLOCKS * _AES_BLOCK_SIZE)
...
{   unsigned char   *ip;
    ...
    if(b_pos)
    {
        memcpy(buf, cbuf, _AES_BLOCK_SIZE);
        if(aes_ecb_encrypt(buf, buf, _AES_BLOCK_SIZE, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;

        while(b_pos < _AES_BLOCK_SIZE && len)
        {
            *obuf++ = *ibuf++ ^ buf[b_pos++];
            --len;
        }
    ...
    while(len)
    {
        ...
        {
            memcpy(ip, cbuf, _AES_BLOCK_SIZE);
            ctr_inc(cbuf);
            ip += _AES_BLOCK_SIZE;
        }

        if(blen & (_AES_BLOCK_SIZE - 1))
            memcpy(ip, cbuf, _AES_BLOCK_SIZE), i++;
...
#endif
        if(aes_ecb_encrypt(buf, buf, i * _AES_BLOCK_SIZE, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;

        i = 0; ip = buf;
# ifdef FAST_BUFFER_OPERATIONS
        if(!ALIGN_OFFSET( ibuf, 4 ) && !ALIGN_OFFSET( obuf, 4 ) && !ALIGN_OFFSET( ip, 4 ))
            while(i + _AES_BLOCK_SIZE <= blen)
            {

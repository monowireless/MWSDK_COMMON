...
extern AES_RETURN aes_encrypt_key(const unsigned char *key, int key_len, aes_encrypt_ctx cx[1]);

#define BLOCK_SIZE      _AES_BLOCK_SIZE      /* block length                 */
...
{   uint_32t cnt;
    ...
    if(
        ...
        tag_len &&
#endif
                   tag_len < 2 /* TODO */ || tag_len > 16 || (tag_len & 1))
...
#else
        else if(ctx->hdr_len < 0x0000000100000000ULL)
        {
